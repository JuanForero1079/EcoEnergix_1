...
export default function HomeUsuario() {
  ...
  const mainActions = [
    {
      title: "Mis Compras",
      description: "Revisa tus pedidos y seguimiento",
      icon: FaShoppingCart,
      path: `/usuario/mis-compras/${usuario?.id || ""}`,
      gradient: "from-[#3dc692] to-[#2aa876]",
      hoverGradient: "from-[#2aa876] to-[#3dc692]",
    },
    {
      title: "Mis Pagos",
      description: "Gestiona tus facturas y métodos de pago",
      icon: FaCreditCard,
      path: `/usuario/mis-pagos/${usuario?.id || ""}`,
      gradient: "from-[#5f54b3] to-[#4a3f8f]",
      hoverGradient: "from-[#4a3f8f] to-[#5f54b3]",
    },
    {
      title: "Soporte Técnico",
      description: "Solicita ayuda especializada",
      icon: FaTools,
      path: `/usuario/soporte/${usuario?.id || ""}`,
      gradient: "from-[#4375b2] to-[#2d5a8f]",
      hoverGradient: "from-[#2d5a8f] to-[#4375b2]",
    },
  ];

  const infoCards = [
    {
      title: "Panel Solar Inteligente",
      description:
        "Monitorea el rendimiento de tus paneles en tiempo real con nuestro sistema IoT integrado.",
      icon: FaSolarPanel,
      color: "text-[#3dc692]",
      bgGradient: "from-[#3dc692]/20 to-[#3dc692]/5",
    },
    {
      title: "Análisis de Consumo",
      description:
        "Obtén reportes detallados de tu consumo energético y recomendaciones personalizadas.",
      icon: FaChartLine,
      color: "text-[#5f54b3]",
      bgGradient: "from-[#5f54b3]/20 to-[#5f54b3]/5",
    },
    {
      title: "Mi Perfil",
      description:
        "Actualiza tu información, cambia contraseña y personaliza tu experiencia.",
      icon: FaUser,
      color: "text-[#4375b2]",
      bgGradient: "from-[#4375b2]/20 to-[#4375b2]/5",
    },
  ];
  ...
  return (
    <section className="relative w-full min-h-screen flex flex-col items-center justify-start px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="absolute inset-0 overflow-hidden -z-10">
        ...
        <div
          className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-[#5f54b3]/20 rounded-full blur-3xl animate-pulse"
      ...
      <div className="max-w-7xl mx-auto w-full space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {quickStats.map((stat, index) => (
            <div
              key={index}
              className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 hover:scale-105 transition-all duration-300 group"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {mainActions.map((action, index) => (
            <Link
              key={index}
              to={action.path}
              className="group relative bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-8 hover:scale-105 transition-all duration-300 overflow-hidden cursor-pointer"
            >
        ...
        <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-3xl p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {infoCards.map((card, index) => (
              <div
                key={index}
                className={`group relative bg-gradient-to-br ${card.bgGradient} border border-white/20 rounded-2xl p-6 hover:scale-105 transition-all duration-300 cursor-pointer`}
              >
