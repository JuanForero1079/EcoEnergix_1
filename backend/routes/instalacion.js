...
router.get("/", (req, res) => {
  DB.query("SELECT * FROM instalacion", (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Error al obtener las instalaciones", details: err });
    res.json(result);
  });
});
  ...
  DB.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al buscar la instalación", details: err });
      if (result.length === 0)
  ...
  DB.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al crear la instalación", details: err });
  ...
  DB.query(
    ...
    (err) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al actualizar la instalación", details: err });
  ...
  DB.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al eliminar la instalación", details: err });
