...
router.get("/", (req, res) => {
  DB.query("SELECT * FROM proveedor", (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Error al obtener los proveedores", details: err });
    res.json(result);
  });
});
  ...
  DB.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al buscar el proveedor", details: err });
      if (result.length === 0)
  ...
  DB.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al crear el proveedor", details: err });
  ...
  DB.query(
    ...
    (err) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al actualizar el proveedor", details: err });
  ...
  DB.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al eliminar el proveedor", details: err });
