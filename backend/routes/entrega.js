...
router.get("/", (req, res) => {
  DB.query("SELECT * FROM entrega", (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Error al obtener entregas", details: err });
    res.json(result);
  });
});
  ...
  DB.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al buscar la entrega", details: err });
      if (result.length === 0)
  ...
  DB.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al crear la entrega", details: err });
...
router.put("/:id", (req, res) => {
  ...
  if (!Fecha_entrega || !Cantidad) {
    return res
      .status(400)
      .json({
        message: "Fecha_entrega y Cantidad son obligatorios para actualizar",
      });
  }
  ...
  DB.query(
    ...
    (err) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al actualizar la entrega", details: err });
...
router.delete("/:id", (req, res) => {
  ...
  DB.query("DELETE FROM entrega WHERE ID_entrega = ?", [id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Error al eliminar la entrega", details: err });

    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ message: "Entrega no encontrada para eliminar" });
    }
