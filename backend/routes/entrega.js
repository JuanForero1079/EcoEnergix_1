...
router.post("/", verificarToken, verificarRol("Administrador"), (req, res) => {
  const { Fecha_entrega, ID_usuario, ID_producto, Cantidad, ID_domiciliario } =
    req.body;

  if (!Fecha_entrega || !ID_usuario || !ID_producto || !Cantidad) {
    return res.status(400).json({
      message: "Todos los campos obligatorios no fueron enviados",
    });
  }
  ...
  DB.query(
    `INSERT INTO entrega 
     ...
     VALUES (?, ?, ?, ?, ?)`,
    [Fecha_entrega, ID_usuario, ID_producto, Cantidad, ID_domiciliario || null],
    (err, result) => {
      if (err) {
        return res.status(500).json({
