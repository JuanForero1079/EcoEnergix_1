...
router.get("/", (req, res) => {
  DB.query("SELECT * FROM soporte_tecnico", (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Error al obtener soportes técnicos", details: err });
    res.json(result);
  });
});
...
router.get("/:id", (req, res) => {
  const { id } = req.params;
  DB.query(
    "SELECT * FROM soporte_tecnico WHERE ID_soporte = ?",
    [id],
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al buscar el soporte técnico", details: err });
      if (result.length === 0)
        return res
          .status(404)
          .json({ message: "Soporte técnico no encontrado" });
      res.json(result[0]);
    },
  );
});
  ...
  DB.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al crear el soporte técnico", details: err });
    ...
    (err) => {
      if (err)
        return res
          .status(500)
          .json({
            error: "Error al actualizar el soporte técnico",
            details: err,
          });
      ...
      res.json({
        message: "Soporte técnico actualizado exitosamente!",
...
router.delete("/:id", (req, res) => {
  ...
  DB.query(
    "DELETE FROM soporte_tecnico WHERE ID_soporte = ?",
    [id],
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({
            error: "Error al eliminar el soporte técnico",
            details: err,
          });
      ...
      if (result.affectedRows === 0) {
        return res
          .status(404)
          .json({ message: "Soporte técnico no encontrado para eliminar" });
      }
