...
router.get("/", (req, res) => {
  DB.query("SELECT * FROM pago", (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Error al obtener los pagos", details: err });
    res.json(result);
  });
});
...
router.get("/:id", (req, res) => {
  ...
  DB.query("SELECT * FROM pago WHERE ID_pago = ?", [id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Error al buscar el pago", details: err });
    if (result.length === 0)
  ...
  DB.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al crear el pago", details: err });
      ...
      res
        .status(201)
        .json({ message: "Pago creado exitosamente!", pago: nuevoPago });
    },
  );
});
  ...
  DB.query(
    ...
    (err) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error al actualizar el pago", details: err });
...
router.delete("/:id", (req, res) => {
  ...
  DB.query("DELETE FROM pago WHERE ID_pago = ?", [id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Error al eliminar el pago", details: err });
