...
const generarAccessToken = (user) =>
  jwt.sign(
    { id: user.ID_usuario, correo: user.Correo_electronico, rol: String(user.Rol_usuario).toLowerCase() },
    JWT_SECRET,
    { expiresIn: "15m" }
  );

const generarRefreshToken = (user) =>
  jwt.sign({ id: user.ID_usuario }, JWT_REFRESH_SECRET, { expiresIn: "7d" });
...
function registrarFallo(correo) {
  ...
  if (intentosFallidos[correo].intentos >= MAX_INTENTOS) {
    intentosFallidos[correo].bloqueadoHasta = new Date(Date.now() + BLOQUEO_MINUTOS * 60000);
  }
}
...
exports.register = async (req, res, next) => {
  try {
    const { Nombre, Correo_electronico, Contraseña, Tipo_documento, Numero_documento, Rol_usuario = "cliente", terms, privacy } = req.body;
...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!user.verificado) throw new AppError("Debes verificar tu correo", 401);
    ...
    res.json({ accessToken, refreshToken, usuario: { id: user.ID_usuario, nombre: user.Nombre, rol: user.Rol_usuario } });
  } catch (error) {
    next(error);
  }
};
...
exports.uploadFoto = async (req, res, next) => {
  try {
    ...
    await new Promise((resolve, reject) => {
      DB.query("UPDATE usuarios SET Foto_usuario = ? WHERE ID_usuario = ?", [rutaFoto, req.user?.id], (err) => (err ? reject(err) : resolve()));
    });
...
exports.refresh = async (req, res, next) => {
  try {
    ...
    const r = await new Promise((resolve, reject) => {
      DB.query("SELECT * FROM usuarios WHERE refresh_token = ? LIMIT 1", [refreshToken], (err, result) => (err ? reject(err) : resolve(result)));
    });

    if (!r[0]) throw new AppError("Refresh token inválido", 401);

    jwt.verify(refreshToken, JWT_REFRESH_SECRET);
    res.json({ accessToken: generarAccessToken(r[0]), refreshToken: generarRefreshToken(r[0]) });
  } catch (error) {
    next(error);
  }
};
...
exports.verifyEmail = async (req, res, next) => {
  try {
    ...
    if (new Date() > r[0].token_expira_en) throw new AppError("El token ha expirado", 400);
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    const user = await new Promise((resolve, reject) => {
      DB.query("SELECT * FROM usuarios WHERE Correo_electronico = ? LIMIT 1", [Correo_electronico], (err, result) => (err ? reject(err) : resolve(result[0])));
    });
    ...
    await new Promise((resolve, reject) => {
      DB.query("UPDATE usuarios SET token_reset = ?, token_reset_expira = ? WHERE ID_usuario = ?", [token, expira, user.ID_usuario], (err) => (err ? reject(err) : resolve()));
    });
...
exports.resetPassword = async (req, res, next) => {
  try {
    ...
    const user = await new Promise((resolve, reject) => {
      DB.query("SELECT * FROM usuarios WHERE token_reset = ? AND token_reset_expira > NOW() LIMIT 1", [token], (err, result) => (err ? reject(err) : resolve(result[0])));
    });
    ...
    const hashed = await hashPassword(Contraseña);
    await new Promise((resolve, reject) => {
      DB.query("UPDATE usuarios SET Contraseña = ?, token_reset = NULL, token_reset_expira = NULL WHERE ID_usuario = ?", [hashed, user.ID_usuario], (err) => (err ? reject(err) : resolve()));
    });
...
exports.deleteAccount = async (req, res, next) => {
  try {
    ...
    if (!userId) throw new AppError("Usuario no autenticado", 401);
    if (!password) throw new AppError("Debes confirmar tu contraseña", 400);

    const user = await new Promise((resolve, reject) => {
      DB.query("SELECT ID_usuario, Contraseña, Foto_usuario FROM usuarios WHERE ID_usuario = ? LIMIT 1", [userId], (err, result) => (err ? reject(err) : resolve(result[0])));
    });

    if (!user) throw new AppError("Usuario no encontrado", 404);

    const isValid = await comparePassword(password, user.Contraseña);
    if (!isValid) throw new AppError("La contraseña no es correcta", 400);

    // Limpiar refresh token
    await new Promise((resolve, reject) => {
      DB.query("UPDATE usuarios SET refresh_token = NULL WHERE ID_usuario = ?", [userId], (err) => (err ? reject(err) : resolve()));
    });

    // Borrar foto si existe
    if (user.Foto_usuario) {
      fs.unlink(`./public${user.Foto_usuario}`, (err) => { if (err) console.log("Error al eliminar foto:", err); });
    }

    // Eliminar usuario
    await new Promise((resolve, reject) => {
      DB.query("DELETE FROM usuarios WHERE ID_usuario = ?", [userId], (err) => (err ? reject(err) : resolve()));
    });
